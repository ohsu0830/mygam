...
"""
    ...
    def update_game(self):
        ...
        for i, e in enumerate(self.enemies):
            ...
            for j, b in enumerate(self.bullets):
                if self.collide(img,b):
                    ...
                    lines = self.enemy_lines.get(s, ["..."])
                    dialogue = random.choice(lines)
                    

                    dialogue_id = self.canvas.create_text(
                        x + self.enemy_imgs[s].width() // 2, 
                        y - 20, 
                        text=dialogue, 
                        font=("Arial", 16, "bold"), 
                        fill="red" if s==10 else "deeppink"
                    )
                    self.window.after(1000, lambda did=dialogue_id: self.canvas.delete(did)) 
        ...
        for j in sorted(list(set(bullets_to_remove)), reverse=True):
             if j < len(bullets_copy):
                self.bullets.remove(bullets_copy[j])
